...
def run_hedging_simulation(paths, config, variance_paths=None):
    ...
    def get_delta_and_price(S_val, v_val, T_rem):
        if config.model_type == "bsm":
            ...
            for i in range(n_paths):
                p_main = heston_price(S_val[i], config.K, T_rem, config.r, v_val[i], config.kappa, config.theta, config.sigma_v, config.rho, config.option_type)
                p_plus = heston_price(S_val[i] + eps[i], config.K, T_rem, config.r, v_val[i], config.kappa, config.theta, config.sigma_v, config.rho, config.option_type)
                prices[i] = p_main
